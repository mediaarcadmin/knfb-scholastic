    ...
    switch (state) {
        ...
        case SCHDictionaryProcessingStateUserDeclined:
        {
            if (isSampleStore) {
                ...
                self.bottomLabel.text = NSLocalizedString(@"You have not yet downloaded the Storia dictionary. To download the dictionary, go to Parent Tools menu.", nil);
            }
        ...
        {
            self.bottomLabel.text = NSLocalizedString(@"The Scholastic dictionary is currently being deleted from your device. Go to Parent Tools menu to download it again.", nil);
            [self.activityIndicator stopAnimating];
            self.progressBar.hidden = YES;
            break;
        }   
