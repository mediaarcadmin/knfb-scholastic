...
- (void)startElement:(const XML_Char *)name attributes:(const XML_Char **)attributes parser:(SCHStoryInteractionParser *)parser
{
    if (strcmp(name, "QuestionPrompt") == 0) {
        ...
        SCHHotSpotCoordinates *hotSpotCoordinates = [[[SCHHotSpotCoordinates alloc] init] autorelease];
        hotSpotCoordinates.rect = CGRectMake([extractXmlAttribute(attributes, "Left") floatValue],
        ...
        [self.hotSpots addObject:hotSpotCoordinates];
...
- (void)endElement:(const XML_Char *)name parser:(SCHStoryInteractionParser *)parser
{
    if (strcmp(name, "Question1") == 0 || strcmp(name, "Question2") == 0 || strcmp(name, "Question3") == 0) {
        ...
        if ([parser.text length] > 0) {
            ...
            SCHHotSpotCoordinates *currentHotSpotCoordinate = [question.hotSpots lastObject];
            if (currentHotSpotCoordinate != nil) {
                [currentHotSpotCoordinate calculatePathWithText:parser.text];
            }
        }
    } else {
        [super endElement:name parser:parser];
