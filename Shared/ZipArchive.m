...
- (void)Percentage:(unsigned long)filePosition;
-(NSDate*) Date1980;

@property unsigned long totalFiles;
...
-(BOOL) UnzipOpenFile:(NSString*) zipFile
{
	...
	if( _unzFile )
	{
		...
		{
            self.totalFiles = globalInfo.number_entry;
			//NSLog([NSString stringWithFormat:@"%d entries in the zip file",globalInfo.number_entry] ); // MATT - commented out this debug message
		} else {
            self.totalFiles = 0;
        }
	}
...
- (void)Percentage:(unsigned long)filePosition
{
    if (self.totalFiles > 0) {
        if( _delegate && [_delegate respondsToSelector:@selector(Percentage:)] )
            [_delegate Percentage:(double)filePosition / (double)self.totalFiles];
    }
}
